                      ...
                      <table  class="display table table-bordered table-striped" id="jnskomoditis">
                        <thead>
                          <tr>
                              ...
                              <th style="text-align:center;" width="80%">Nama Kategori Titik Pantau</th>
                              <th style="text-align:center;" width="10%">Aksi</th>
                          </tr>
                          </thead>
                          <tbody>
                          <?php $no = 1;?>
                          @foreach($kattipas as $value)
                            <tr>
                                <td style="text-align:center;"><?php echo $no ?></td>
                                <td>{{{ $value->nama }}}</td>
                                <td style="text-align:center;">
                                  <div class="btn-group">
                                    <a href="{{ route('admin.kattipas.edit', ['kattipas'=>Crypt::encrypt($value->id)]) }}" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
                                    {{ Form::open(array('url'=>route('admin.kattipas.destroy',['kattipas'=>$value->id]),'method'=>'delete', 'style'=>'display:inline;')) }}
