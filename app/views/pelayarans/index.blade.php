    ...
    <div class="col-lg-12">
        <section class="panel">
            <header class="panel-heading">
                Daftar Mitra Pelayaran
                {{ HTML::buttonAdd() }}
            </header>
            <div class="panel-body"  style:"overflow: scroll;">
                  <div class="adv-table">
                      <table  class="display table table-bordered table-striped" id="pelayarans">
                        <thead>
                          <tr>
                              <th style="text-align:center;" width="10%">No</th>
                              <th style="text-align:center;" width="80%">Nama Mitra Pelayaran</th>
                              <th style="text-align:center;" width="10%">Aksi</th>
                          </tr>
                          </thead>
                          <tbody>
                          <?php $no = 1;?>
                          @foreach($pelayarans as $value)
                            <tr>
                                <td style="text-align:center;"><?php echo $no ?></td>
                                <td>{{{ $value->nama }}}</td>
                                <td style="text-align:center;">
                                  <div class="btn-group">
                                    <a href="{{ route('admin.pelayarans.edit', ['pelayarans'=>Crypt::encrypt($value->id)]) }}" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
                                    {{ Form::open(array('url'=>route('admin.pelayarans.destroy',['pelayarans'=>$value->id]),'method'=>'delete', 'style'=>'display:inline;')) }}
